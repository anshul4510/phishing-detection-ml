...
class DataValidation:
    ...
    def validate_number_of_numerical_columns(self,dataframe:pd.DataFrame)->bool:
        try:
            ...
            number_of_numerical_columns=len(required_numerical_columns)
            logging.info(f"Required number of numerical columns: {number_of_numerical_columns}")
            logging.info(f"Dataframe has numerical columns: {len(dataframe.select_dtypes(include=['int64', 'float64']).columns)}")

            if len(dataframe.select_dtypes(include=['int64', 'float64']).columns) == number_of_numerical_columns:
                return True
            return False
    ...
    def initiate_data_validation(self)->DataValidationArtifacts:
        try:
            ...
            if not status:
                error_message=f"Train datframe doesnot contain all columns.\n"
            status=self.validate_number_of_columns(dataframe=test_dataframe)
            if not status:
                error_message=f"Test datframe doesnot contain all columns.\n"
            status=self.validate_number_of_numerical_columns(dataframe=train_dataframe)
            if not status:
                error_message=f"Train datframe doesnot contain numeric columns.\n"
            status=self.validate_number_of_numerical_columns(dataframe=test_dataframe)
            if not status:
                error_message=f"Test datframe doesnot contain numeric columns.\n"
